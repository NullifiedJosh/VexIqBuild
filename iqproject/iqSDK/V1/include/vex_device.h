/*----------------------------------------------------------------------------*/
/*                                                                            */
/*    Copyright (c) Innovation First 2017, All rights reserved.               */
/*                                                                            */
/*    Module:     vex_device.h                                                */
/*    Author:     James Pearman                                               */
/*    Created:    21 Nov 2016                                                 */
/*                                                                            */
/*    Revisions:                                                              */
/*                V1.00     TBD - Initial release                             */
/*                                                                            */
/*----------------------------------------------------------------------------*/

#ifndef   VEX_DEVICE_CLASS_H
#define   VEX_DEVICE_CLASS_H

/*-----------------------------------------------------------------------------*/
/** @file    vex_device.h
  * @brief   Header for device class
*//*---------------------------------------------------------------------------*/

/*-----------------------------------------------------------------------------*/
/** @brief device class                                                        */
/*-----------------------------------------------------------------------------*/
namespace vex {
  class device  {
    private:
      int32_t         _lastpolltime;
      int16_t         _pollinterval;

    protected:
      int16_t         _index;
      void            setPollInterval( int32_t value );
      bool            pollValid( bool bSave );
    
    public:
      device();
      device( int32_t index );  
      ~device();
      
      IQ_DeviceType   type();
      int32_t         index();
      void            init( int32_t index );
      virtual bool    installed();
      virtual int32_t value();
      
      int32_t         readDigitalPin();
      int32_t         readAnalogPin();
  };
};

/*-----------------------------------------------------------------------------*/
/** @brief a safe array template                                               */
/*-----------------------------------------------------------------------------*/
// This version of safearray now returns element 0 for out of bounds access
// The only place we actually use this on IQ is in vision class
// need to save memory !
namespace vex {
  template <class T, int len> class safearray {
    private:
      int length;
  
    protected:
      T arr[len];
  
    public:
      safearray(){ length = len;
      };
      ~safearray(){};
  
      T &operator[]( int i );
      int getLength() { return length; };
  };

  template <class T, int len> T &safearray<T, len>::operator[](int i)
  {
    // bounds check the array index
    if( i < 0 || i > (length-1) ) {
      return( arr[0] );
    }
    else {
      return( arr[i] );
    }
  }
};

/*-----------------------------------------------------------------------------*/
/** @brief All devices class                                                   */
/*-----------------------------------------------------------------------------*/
namespace vex {
  /** @brief Use the device class to get information about all of the vex devices plugged into the IQ.
  */
  class devices {
    private:
      //safearray<device, IQ_MAX_DEVICE_PORTS> data;

    public:
      devices();  
      ~devices();

      //device &operator[]( int i ) {
      //  return( data[i] );
      //}
      /**
      * @brief Get the V5 device type plugged into a specific port.      
      * @return Returns a V5 device type.
      * @param index Specifies the index to look at for the device.
      */
      IQ_DeviceType   type( int32_t index );

      /**
      * @brief Gets the number of VEX devices that are plugged in.    
      * @return Returns an integer that represent the number of vex devices
      */
      int32_t         number();

      /**
      * @brief Gets the number of specified devices that are plugged into the V5.
      * @return Returns an integer that represents the number of a specific vex devices set by the parameter.
      * @param  type The type of device to look for on the V5.
      */
      int32_t         numberOf( IQ_DeviceType type );
  };
};

#endif // VEX_DEVICE_CLASS_H
